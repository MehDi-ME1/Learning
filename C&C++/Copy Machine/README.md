## Review
A copy file utility which uses low-level system calls. 
It can be used as a command in terminl using this "./filename \<source> \<destination>".
Concept used in this program is simple, the streams and error handlins are solid.
Also a good practice for main arguments.

### How to Run
Before using it as command in terminl, it should be compiled with any gcc compiler.
